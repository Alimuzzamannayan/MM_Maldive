...
<head><meta charset="UTF-8"><style>
  ...
  .header h1 { color: #00E5C0; font-size: 20px; margin: 0; font-family: sans-serif; }
  ...
  .message-box { background: #f8fffe; border: 1px solid #00E5C020; border-radius: 8px; padding: 16px; margin-top: 20px; color: #333; font-size: 14px; line-height: 1.6; white-space: pre-wrap; }
...
<div class="wrap">
  <div class="header">
    ...
    <p>Submitted via mm-maldive.vercel.app — ${new Date().toUTCString()}</p>
  </div>
  <div class="body">
    ...
    ${message ? `<div class="message-box">${message.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>` : ""}
  </div>
  <div class="footer">Metamorphosis MV — automated notification</div>
</div>
</body></html>`;

  try {
    await transporter.sendMail({
      from: `"Metamorphosis MV Website" <${process.env.SMTP_USER}>`,
